...
class ApprovalManager:
    ...
    async def request_approval(self, operation: str, details: dict[str, Any]) -> bool:
        ...
        # 1. YOLO 模式：全部放行
        if self._yolo_mode:
            logger.debug("YOLO 模式自动批准", operation=operation)
            return True

        # 2. 不在需审批列表中：放行
        if operation not in self.require_approval_for:
            logger.debug("操作无需审批", operation=operation)
            return True

        # 3. 本会话已自动批准同类操作：放行
        if operation in self._auto_approved:
            logger.debug("会话级自动批准", operation=operation)
            return True

        # 4. 显示面板并读取用户选择
        logger.info("请求用户审批", operation=operation)
        return self._prompt_user(operation, details)

    def enable_yolo_mode(self) -> None:
        """开启 YOLO 模式，后续所有操作自动批准。"""
        self._yolo_mode = True
        logger.warning("YOLO 模式已开启，后续所有操作自动批准")

    # ------------------------------------------------------------------
    # 内部实现
    # ------------------------------------------------------------------

    def _prompt_user(self, operation: str, details: dict[str, Any]) -> bool:
        ...
        while True:
            try:
                choice = (
                    input("批准? [Y]es / [N]o / [A]lways / [!]YOLO > ").strip().lower()
                )
            except (KeyboardInterrupt, EOFError):
                # Ctrl+C / Ctrl+D 视为拒绝
                self._console.print("[yellow]已拒绝（中断）[/yellow]")
                logger.info("用户中断审批", operation=operation)
                return False

            if choice in ("y", "yes"):
                logger.info("用户批准操作", operation=operation)
                return True
            if choice in ("n", "no"):
                logger.info("用户拒绝操作", operation=operation)
                return False
            if choice == "a":
                ...
                logger.info("会话级自动批准已开启", operation=operation)
                return True
            if choice == "!":
                self._yolo_mode = True
                self._console.print(
                ...
                logger.warning("用户开启 YOLO 模式")
                return True
            # 无效输入，重新提示
            self._console.print("[yellow]无效选择，请输入 y/n/a/! [/yellow]")

    def _render_panel(self, operation: str, details: dict[str, Any]) -> None:
        """渲染审批面板。
        ...
        if operation in ("file_write", "file_edit"):
            ...
            if content:
                preview = (
                    ...
                    else content[:_MAX_CONTENT_PREVIEW] + "\n...[truncated]"
                )
                content_panel = Panel(
                    Text(preview),
                    title="内容预览",
