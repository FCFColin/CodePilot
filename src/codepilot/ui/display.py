...
实现 DisplayManager 类，提供 AgentLoop 需要的所有 UICallback 回调方法，
...
from codepilot.config import Config, UIConfig
from codepilot.context.manager import ContextStats
...
_DEEPSEEK_PRICING: dict[str, float] = {"input": 0.14, "output": 0.28}
# Anthropic Claude 定价（参考官方公开价格）
_ANTHROPIC_PRICING: dict[str, float] = {"input": 3.0, "output": 15.0}
...
def _mask_api_key(api_key_str: str) -> str:
    """掩码 API Key，仅显示后 4 位。"""
    if not api_key_str:
        return "(未设置)"
    if len(api_key_str) <= 4:
        return "****"
    return "***" + api_key_str[-4:]


class DisplayManager:
    """UI 显示管理器，实现 agent.loop.UICallback Protocol。
    ...
    同时提供 slash 命令所需的显示方法（show_help/show_config 等）。
    """
    ...
    # ------------------------------------------------------------------
    # UICallback Protocol 方法（async，由 AgentLoop 调用）
    # ------------------------------------------------------------------
    ...
    def _print_usage_panel(self) -> None:
        ...
        if self.config.show_cost_estimate:
            cost = _estimate_cost(
                self._cumulative_input,
                self._cumulative_output,
                self.provider_name,
            )
        ...
        content = Text()
        content.append(
            f"   Input: {self._cumulative_input:,} | "
            f"Output: {self._cumulative_output:,} | "
            f"Total: {cumulative_total:,}\n",
            style="bold",
        )
        if max_tokens > 0:
            content.append(
                f"   Context: {context_tokens:,} / {max_tokens:,} "
                f"({usage_percent:.1f}%)",
                style="cyan",
            )
        else:
    ...
    async def on_error(self, error: str) -> None:
        """显示错误信息（红色）。"""
        self._stop_live()
        content = Text(f"❌ {error}", style="bold red")
        panel = Panel(
            content,
            title="Error",
            border_style="red",
    ...
    def on_compression(self, stats: dict[str, Any]) -> None:
        """显示上下文压缩通知面板。"""
        self._stop_live()
        ...
        content.append(
            Text(f"Compressed: {messages_compressed} messages\n", style="cyan")
        )
    ...
    def show_stats(self, stats: ContextStats) -> None:
        """显示详细统计（/stats 命令）。

        Args:
            stats: ContextStats 统计字典。
        """
        self._stop_live()
        ...
        cost = _estimate_cost(input_tokens, output_tokens, self.provider_name)
        ...
        content.append(Text(f"  压缩次数:    {compression_count}\n\n"))
    ...
    def show_config(self, config: Config) -> None:
        ...
        content.append(
            Text(f"  require_approval_for: {config.security.require_approval_for}\n")
        )
        content.append(
            Text(f"  auto_approve_read: {config.security.auto_approve_read}\n")
        )
        content.append(
            Text(
                f"  command_whitelist_mode: {config.security.command_whitelist_mode}\n"
            )
        )
        content.append(Text("\n"))
        ...
        content.append(
            Text(f"  compression_threshold: {config.context.compression_threshold}\n")
        )
        content.append(
            Text(f"  critical_threshold:    {config.context.critical_threshold}\n")
        )
        content.append(
            Text(f"  preserve_recent_turns: {config.context.preserve_recent_turns}\n")
        )
        content.append(
            Text(f"  compression_strategy:  {config.context.compression_strategy}\n")
        )
        content.append(
            Text(f"  save_full_history:     {config.context.save_full_history}\n")
        )
        content.append(Text("\n"))
    ...
    def show_help(self) -> None:
        ...
        commands: list[tuple[str, str]] = [
            ...
            ("/undo", "撤销最近的文件操作（可连续执行多步）"),
            ("/rollback <N>", "回退到第 N 轮对话"),
            ("/plan", "显示当前执行计划"),
