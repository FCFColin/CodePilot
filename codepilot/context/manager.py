...
class ContextManager:
    ...
    async def maybe_compress(self) -> dict | None:
        """检查是否需要压缩，需要则触发。
    ...
    async def _do_compress(self) -> dict:
        """实际执行压缩（调用方已持有锁）。
    ...
    def _truncate_fallback(self, compressible: list[Message]) -> tuple[str, dict]:
    ...
    def get_stats(self) -> dict:
        ...
        usage_percent = 0.0
        if self.config.max_tokens > 0:
            usage_percent = self.total_tokens / self.config.max_tokens
        return {
